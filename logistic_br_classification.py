...
out_csv_name = f'../{project}_LR.csv' # saves to project folder's parent directory
...
#test array to check if values are being stored
testArray = []
...
array = np.array(f1_scores)
np.save("f1_scores_LR.npy", array)
...
np.save("data_LR.npy",array)
arrayWeReadIn = np.load("data_LR.npy").tolist()
print("Array we read in from a file: ",arrayWeReadIn)'''
